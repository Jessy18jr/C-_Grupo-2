...
/*******************************************
 * 1. Definición de tokens (Análisis Léxico)
 *******************************************/
...
struct Token {
    ...
    int line;    // línea de código donde aparece
...
// Analizador Léxico (Scanner) mejorado
class Lexer {
private:
    string input;
    size_t pos;
...
public:
    ...
    // Avanza en el input mientras haya espacios, tabs o saltos de línea
    void skipWhitespace() {
    ...
    // Salta comentarios de una línea y multilínea
    void skipComments() {
    ...
    Token getNextToken() {
        ...
        if (c == '"') {
            ...
            while (pos < length && input[pos] != '"') {
                ...
                if (input[pos] == '\\' && pos + 1 < length) {
                    pos += 2;  // Saltar el carácter de escape y el siguiente
                    column += 2;
                } else {
                    if (input[pos] == '\n') {
                        line++;
        ...
        if (c == '\'') {
            ...
            if (pos < length && input[pos] == '\\') {
                pos += 2;  // Saltar la barra invertida y el carácter de escape
                column += 2;
            } else if (pos < length) {
                pos++;
                column++;
        ...
        // Operadores de doble carácter y otros tokens
        if (pos + 1 < length) {
...
/***************************************
 * 2. Estructuras para el Análisis Sintáctico (AST)
 ****************************************/
...
// Expresión para números flotantes
...
public:
    ...
    string getType() const override {
        ...
        // Para operadores aritméticos, el tipo resultante depende de los operandos
...
// Expresión unaria (ej: -5, !expresion)
class UnaryExpr : public Expr {
public:
    enum OpType {
        OP_NEG,  // Negación aritmética (-)
...
// Expresión de llamada a función (ej: printf("hola"))
...
// Declaración de variable: int a;
class VarDeclStmt : public Stmt {
public:
    string type;     // Tipo de la variable (int, float, etc.)
    string varName;  // Nombre de la variable
    unique_ptr<Expr> initializer; // Expresión de inicialización, si existe
...
// Expresión como sentencia (por ejemplo, llamadas a funciones): printf("hola");
...
public:
    ...
    vector<VarDeclStmt*> parameters; // Parámetros
    unique_ptr<BlockStmt> body;      // Cuerpo de la función
...
/***************************************
 * 3. Parser (Análisis Sintáctico y Semántico)
 ****************************************/
...
private:
    Lexer &lexer;
    Token currentToken;
    map<string, string> symbolTable; // Tabla de símbolos: nombre -> tipo

    // Función auxiliar para consumir el token actual y avanzar
    void advance() {
    ...
    unique_ptr<Program> parseProgram() {
        ...
        // Procesar directivas de inclusión
        while (currentToken.type == T_POUND) {
            ...
            if (currentToken.type == T_INCLUDE) {
                ...
                throw runtime_error("Error de sintaxis: se esperaba 'include' después de '#'");
            }
        }
    ...
    IncludeDirective* parseInclude() {
        ...
        if (currentToken.type == T_LT) {
            ...
            throw runtime_error("Error de sintaxis: se esperaba '<' o '\"' después de include");
        }
    ...
    FunctionDecl* parseFunctionDefinition() {
        ...
        // Procesar parámetros si hay alguno
    ...
    Stmt* parseStatement() {
        if (currentToken.type == T_INT || currentToken.type == T_FLOAT || currentToken.type == T_CHAR) {
            ...
            // Puede ser una asignación o una llamada a función
    ...
    Stmt* parseCinStatement() {
        ...
        do {
            ...
            if (currentToken.type != T_ID) {
                throw runtime_error("Error de sintaxis: se esperaba un identificador después de '>>'");
            }
    ...
    Expr* parseAssignment() {
        ...
        if (currentToken.type == T_ASSIGN) {
            ...
            delete expr; // Ya no necesitamos la expresión original
    ...
    CallExpr* parseCallExprWithId(const string &funcName) {
        ...
        // se usaría una tabla de símbolos de funciones)
...
public:
    ...
    // Genera código para una expresión y retorna el nombre de la variable donde se guarda el resultado
    ...
    void genStmt(Stmt* stmt, vector<string> &code) {
        if (auto decl = dynamic_cast<VarDeclStmt*>(stmt)) {
            ...
            throw runtime_error("Sentencia desconocida en generación");
        }
    }
    ...
    void genFunction(FunctionDecl* func, vector<string> &code) {
        ...
        // Si es void y no tiene return explícito, añadimos uno
    ...
    // Genera código para una directiva include
...
/***************************************
 * 5. Evaluación y Ejecución
 ****************************************/

// Esta clase es una versión simplificada de un intérprete
// para el código intermedio, solo para demostrar la funcionalidad
class Interpreter {
private:
    ...
    int evaluateExpr(const string &expr) {
        ...
        // No podemos manejar expresiones complejas en esta versión simplificada
        throw runtime_error("No se puede evaluar la expresión: " + expr);
    }
    ...
    void execute(const vector<string> &code) {
        for (const auto &line : code) {
            if (line.find("DECL") == 0 || line.find("//") == 0 || 
                ...
                // Ignorar declaraciones, comentarios, declaraciones de función, etc.
                continue;
            ...
            // Otras instrucciones (if, goto, etc.) no se manejan en esta versión simplificada
        }
    }
...
int main() {
    ...
    // Método alternativo para leer la entrada con mejor detección de Ctrl+Z en Windows
    string sourceCode = "";
    string line;
    ...
    while (true) {
        ...
        sourceCode += line + "\n";
        cout << "> "; // Mostrar indicador para cada nueva línea
    }
    
    // Si no hay entrada o solo se presionó Ctrl+Z sin código
    ...
    try {
        ...
        // 4. Interpretación/Ejecución simplificada (opcional)
        cout << "\n=== Ejecucion Simulada ===\n";
